...
class AudioAttack():
    ...
    def generate(self, x: Union[torch.Tensor, np.ndarray], y: Union[torch.Tensor, np.ndarray], targeted: bool=False):
        ...
        for i in range(0, self.max_iter_1 + 1):
            ...
            if self._targeted:
                ...
                if self.norm == 'linf':
                    ...
                    # grad will under flow so we need to clip it
                    grad = torch.clamp(grad, min=1e-10, max=1e10)
            ...
            delta.data = torch.cat([project_to_norm_ball(torch.unsqueeze(p, 0), self.norm, e) for p, e in zip(delta.data, epsilon)], dim=0)
