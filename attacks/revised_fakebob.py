...
class FakeBob(object):
    ...
    def __init__(self, task, attack_type, model, adver_thresh=0., epsilon=0.002, max_iter=1000, 
                 max_lr=0.001, min_lr=1e-6, samples_per_draw=50, samples_per_draw_batch_size=2, sigma=0.001, momentum=0.9, 
    ...
    def generate(self, x, threshold=0., y=None, target=None, fs=16000, 
               bits_per_sample=16, n_jobs=10, debug=False):
        x = np.float32(x.cpu().numpy())
        y = np.int64(y.cpu().numpy())
        ...
        lower = np.clip(x - self.epsilon, -1., 1.)
        upper = np.clip(x + self.epsilon, -1., 1.)
        ...
        for iter in range(self.max_iter):
            ...
            distance = np.max(np.abs(x - adver))
        ...
        adver = torch.from_numpy(adver).float()
    ...
    def get_grad(self, audio, fs=16000, bits_per_sample=16, n_jobs=10, debug=False):
        ...
        N = audio.size
        repeat_times = self.samples_per_draw // self.samples_per_draw_batch_size
        for _ in range(repeate_times):
            noise_pos = np.random.normal(size=(N, self.samples_per_draw_batch_size // 2))
