...
class RevVPSDE(torch.nn.Module):
    ...
    def vpsde_fn(self, t, x):
        ...
        disc_steps = self._scale_timesteps(t).to(x.device) - 1 # debug: using discrete params
        beta_t = self.discrete_betas.to(x.device)
        beta_t = beta_t[disc_steps] * self.N # debug: using discrete params
    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        if return_type == 'drift':
            ...
            disc_steps = self._scale_timesteps(t).to(x.device) - 1
            if disc_steps.unique() > 0:
                alphas = self.alphas_cumprod.to(x.device)
                scale_factor = torch.sqrt(1 - alphas[disc_steps - 1]) / torch.sqrt(1 - alphas[disc_steps])
...
class RevDiffWave(torch.nn.Module):
    def __init__(self, args, device=None):
        ...
        self.rev_vpsde.model.to(self.device)
