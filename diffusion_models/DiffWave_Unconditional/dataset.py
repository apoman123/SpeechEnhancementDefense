...
class QUALCOMMKEYWORD(Dataset):
    ...
    def __init__(self, root: str, 
        ...
        self.qkws = load_from_disk(root).cast_column("audio", Audio(sampling_rate=16000))['train']

    def __len__(self):
        return len(self.qkws)
    
    def __getitem__(self, index):
        data = self.qkws[index]
        data = {'samples': data['audio']['array'].astype(np.float32), "sample_rate": data['audio']['sampling_rate'], 'target': data['label']}
        return data
        
class VCTK(Dataset):
    def __init__(self, path, transform=LoadAudio()):
        self.file_paths = []
        classes = os.listdir(path)
        for label in classes:
            for file in os.listdir(os.path.join(path, label)):
                self.file_paths.append((os.path.join(path, label, file), int(label))) 
        self.transform = transform
    def __len__(self):
        return len(self.file_paths)

    def __getitem__(self, idx):
        data = {'path': self.file_paths[idx][0], 'target': self.file_paths[idx][1]}
        data = self.transform(data)
        return data
...
def load_VCTK(path, batch_size=4, num_gpus=1):
    vctk = VCTK(path)
    
    # distributed sampler
    train_sampler = DistributedSampler(vctk) if num_gpus > 1 else None

    trainloader = torch.utils.data.DataLoader(vctk, 
                                              batch_size=batch_size,  
                                              sampler=train_sampler,
                                              collate_fn=rand_audio_length_collate, 
                                              num_workers=10,
