...
def get_classifier(args):
    if args.dataset == "sc09":
        from victim_models import M5
        from defense_modules import M5_Features
        victim = M5(n_input=1, n_output=10)
        transform = M5_Features(zero_mean=args.zero_mean)
...
#     print('----- Training Loop -----')
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(surrogate_model.state_dict(), os.path.join(args.checkpoint_path, f"m5net_{epoch}.pt"))
        print("Model Saved!")
