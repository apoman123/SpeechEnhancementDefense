...
def get_z_sets(model, data, lr, loss, device, rec_iter = 200, rec_rr = 10, input_latent = 100, global_step = 1):
    ...
    for idx in range(len(z_hats_recs)):
        ...
        for iteration in range(rec_iter):
            ...
            fake_wave = model(z_hat)
            
            # fake_wave = fake_wave.view(-1, data.size(1), data.size(2))

            # padding
            # fake_image = torch.cat([fake_image, torch.zeros(data.size(0), data.size(1), spec_shape - fake_image.size(-2), fake_image.size(-1)).to(fake_image.device)], dim=-2)
            # fake_image = torch.cat([fake_image, torch.zeros(data.size(0), data.size(1), spec_shape, spec_shape - fake_image.size(-1)).to(fake_image.device)], dim=-1)
            
            reconstruct_loss = loss(fake_wave, data)
