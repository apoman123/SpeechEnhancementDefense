# This is the diffusion model we use
...
class RevVPSDE(torch.nn.Module):
    """
    VP-SDE model: diffusion model
    guided_diffusion: For ImageNet
    score_sde: For CIFAR-10
    """

    def __init__(self, model, score_type='guided_diffusion', beta_min=0.1, beta_max=20, N=1000,
                 img_shape=(1, 256, 256), model_kwargs=None):
    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        if return_type == 'drift':
            ...
            if self.score_type == 'guided_diffusion':
                ...
                model_output = self.model(x_img, disc_steps, **self.model_kwargs)  # [10, 3, 32, 32]
                # with learned sigma, so model_output contains (mean, val)
                model_output = torch.split(model_output, self.img_shape[0], dim=1)[0]
...
class RevGuidedDiffusion(torch.nn.Module):
    def __init__(self, args, config, device=None):
        ...
        if config.data.dataset == 'CIFAR10':
            ...
            model.load_state_dict(torch.load(f'{model_dir}/model_64_200000.pt', map_location='cpu'))
            if model_config['use_fp16']:
                model.convert_to_fp16()
        elif config.data.dataset == '_128':
            img_shape = (3, 128, 128)
            model_dir = 'pretrained/guided_diffusion'
            model_config = model_and_diffusion_defaults()
            model_config.update(vars(self.config.model))
            print(f'model_config: {model_config}')
            model, _ = create_model_and_diffusion(**model_config)
            if config.model.freq == 0:
                model.load_state_dict(torch.load(f'{model_dir}/model_128_800000.pt', map_location='cpu'))
            elif config.model.freq == 1:
                model.load_state_dict(torch.load(f'{model_dir}/spectop128_600000.pt', map_location='cpu'))
            if model_config['use_fp16']:
                model.convert_to_fp16()
                
        elif config.data.dataset == '_256':
            img_shape = (3, 256, 256)
            model_dir = '/home/apoman123/speech_enhancement_adversarial_evaluation/wavepurifier_components/pretrained/guided_diffusion'
            model_config = model_and_diffusion_defaults()
            model_config.update(vars(self.config.model))
            print(f'model_config: {model_config}')
            model, _ = create_model_and_diffusion(**model_config)
            # model.load_state_dict(torch.load(f'{model_dir}/model_256_200000.pt', map_location='cpu'))
            model.load_state_dict(torch.load(f'{model_dir}/model100000.pt', map_location='cpu'))

            if model_config['use_fp16']:
                model.convert_to_fp16()

        elif config.data.dataset == "sc09":
            img_shape = (1, 256, 256)
            model_dir = '/home/apoman123/data/nas07/PersonalData/apoman123/wavepurifier_sc09'
            model_config = args_to_dict(args, model_and_diffusion_defaults().keys())
            model, diffusion = create_model_and_diffusion(**args_to_dict(args, model_and_diffusion_defaults().keys()))
            print(f'model_config: {args_to_dict(args, model_and_diffusion_defaults().keys())}')
            # model.load_state_dict(torch.load(f'{model_dir}/model_256_200000.pt', map_location='cpu'))
            model.load_state_dict(torch.load(f'{model_dir}/model100000.pt', map_location='cpu'))
        ...
        elif config.data.dataset == "qkws":
            img_shape = (1, 256, 256)
            model_dir = '/home/apoman123/data/nas07/PersonalData/apoman123/wavepurifier_qkws'
            model_config = args_to_dict(args, model_and_diffusion_defaults().keys())
            model, diffusion = create_model_and_diffusion(**args_to_dict(args, model_and_diffusion_defaults().keys()))
            print(f'model_config: {args_to_dict(args, model_and_diffusion_defaults().keys())}')
            # model.load_state_dict(torch.load(f'{model_dir}/model_256_200000.pt', map_location='cpu'))
        ...
        elif config.data.dataset == "vctk":
            img_shape = (1, 256, 256)
            model_config = args_to_dict(args, model_and_diffusion_defaults().keys())
            model_dir = '/home/apoman123/data/nas07/PersonalData/apoman123/wavepurifier_vctk_256_1000'
            model, diffusion = create_model_and_diffusion(**args_to_dict(args, model_and_diffusion_defaults().keys()))
            print(f'model_config: {args_to_dict(args, model_and_diffusion_defaults().keys())}')
            # model.load_state_dict(torch.load(f'{model_dir}/model_256_200000.pt', map_location='cpu'))
            model.load_state_dict(torch.load(f'{model_dir}/model100000.pt', map_location='cpu'))
    ...
    def image_editing_sample(self, img, window_id, bs_id=0):
        ...
        # out_dir = os.path.join(self.args.log_dir, tag)
        ...
        #     tvu.save_image((x0 + 1) * 0.5, os.path.join(out_dir, f'original_input_{str(window_id)}.png'))

        # xs = []
        for it in range(self.args.sample_step):
            # print ("sample step: {}".format(it))
            e = torch.randn_like(x0).to(self.device)
            total_noise_levels_s = self.args.t_s
            total_noise_levels_m = self.args.t_m
            total_noise_levels_l = self.args.t_l
        
            if self.args.rand_t:
                total_noise_levels_s = self.args.t_s + np.random.randint(-self.args.t_delta, self.args.t_delta)
                total_noise_levels_m = self.args.t_m + np.random.randint(-self.args.t_delta, self.args.t_delta)
                total_noise_levels_l = self.args.t_l + np.random.randint(-self.args.t_delta, self.args.t_delta)
                # print(f'total_noise_levels: {total_noise_levels}')
            # print (self.betas.shape)
            a = (1 - self.betas).cumprod(dim=0).to(self.device)
            xs = x0 * a[total_noise_levels_s - 1].sqrt() + e * (1.0 - a[total_noise_levels_s - 1]).sqrt()  # refer to Eq.3
            xm = x0 * a[total_noise_levels_m - 1].sqrt() + e * (1.0 - a[total_noise_levels_m - 1]).sqrt()  # refer to Eq.3
            xl = x0 * a[total_noise_levels_l - 1].sqrt() + e * (1.0 - a[total_noise_levels_l - 1]).sqrt()  # refer to Eq.3

            # Save the noisy specs
            # if bs_id < 2:
            #     tvu.save_image((xs + 1) * 0.5, os.path.join(out_dir, f'init_{self.args.t_s}_{str(window_id)}.png'))
            #     tvu.save_image((xm + 1) * 0.5, os.path.join(out_dir, f'init_{self.args.t_m}_{str(window_id)}.png'))
            #     tvu.save_image((xl + 1) * 0.5, os.path.join(out_dir, f'init_{self.args.t_l}_{str(window_id)}.png'))

            epsilon_dt0, epsilon_dt1 = 0, 1e-5
            # # track backwards Hanqing 01/23/2023
            # self.args.t = 200
            purified_x = []
            for x in [xs, xm, xl]:
                if x is xs:
                    ...
                    t = self.args.t_l
                t0, t1 = 1 - t * 1. / 1000 + epsilon_dt0, 1 - epsilon_dt1
                t_size = 2
                ...
                if self.args.use_bm:
                    ...
                    # The sdeint_adjoint function require the rev_vpsde model has forward and reverse step
                    # Refer to https://github.com/google-research/torchsde
                    xs_ = torchsde.sdeint_adjoint(self.rev_vpsde, x_, ts, method='euler', bm=bm)  # refer to Eq. 6
                else:
                    xs_ = torchsde.sdeint_adjoint(self.rev_vpsde, x_, ts, method='euler')
                x0 = xs_[-1].view(x.shape)  # (batch_size, c, h, w)
                purified_x.append(x0)
                # if bs_id < 2:
                #     # torch.save(x0, os.path.join(out_dir, f'samples_{it}.pth'))
                #     tvu.save_image((x0 + 1) * 0.5, os.path.join(out_dir, f'samples_{t}_{str(window_id)}.png'))
                    # tvu.save_image((x0 + 1) * 0.5, os.path.join(out_dir, f'middle_{str(self.args.t)}_{str(window_id)}.png'))

                # xs.append(x0)
            
            return torch.cat([purified_x[0][:, :, :64, :], purified_x[1][:, :, 64:128, :], purified_x[2][:, :, 128:, :]], dim=2)
